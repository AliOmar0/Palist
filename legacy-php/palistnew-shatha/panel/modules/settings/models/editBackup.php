...
//Example Usage/s
trash(backup_dir);
$backup = new BackupMyProject(cms_dir);
...
class BackupMyProject{
    ...
    function __construct($path=null, $download=false){
        ...
        $this->backup_file  = self::PWD.basename($this->project_path).'__'.date("Y-m-d H:i").'.zip';
		...
		$data="
		<li class=\"recent_backup\"><a href='".backup_url.basename($this->project_path).'__'.date("Y-m-d H:i").'.zip'."'>".basename($this->project_path).'__'.date("Y-m-d H:i").'.zip'."</a></li>";
			
		foreach (scandir(backup_dir) as $file) {
		$extension = pathinfo($file, PATHINFO_EXTENSION);
		if ($extension == 'zip') {
	$data=$data. "<li><a href=\"".backup_url.$file."\" title=\"Download\">".$file."</a></li>";
		...
		// json(true,2,$data,NULL,['js'=>'updateBackUpList']);
        j();
	}
