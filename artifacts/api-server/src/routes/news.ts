...
router.get("/news", async (req, res) => {
  ...
  const where = q
    ? and(
        ...
        or(
          ilike(newsTable.titleAr, `%${q}%`),
          ilike(newsTable.titleEn, `%${q}%`),
          ilike(newsTable.summaryAr, `%${q}%`),
          ilike(newsTable.summaryEn, `%${q}%`),
          ilike(newsTable.contentAr, `%${q}%`),
          ilike(newsTable.contentEn, `%${q}%`),
        ),
      )
    : eq(newsTable.published, true);
  const rows = await db
...
router.post("/admin/news", requireAdmin, auditAdmin("news"), async (req, res) => {
...
router.patch("/admin/news/:id", requireAdmin, auditAdmin("news"), async (req, res) => {
  const id = Number(req.params.id);
  if (!Number.isFinite(id)) return res.status(400).json({ error: "Bad id" });
...
router.delete("/admin/news/:id", requireAdmin, auditAdmin("news"), async (req, res) => {
  const id = Number(req.params.id);
  if (!Number.isFinite(id)) return res.status(400).json({ error: "Bad id" });
