<?php 
$original_edit_id=$id=check_get_id();
	$_form_resp=db('sms_1583164170','WHERE id=\''.$id.'\'',NULL,'LIMIT 1');
if(!privilege('sms_1583164170','edit') || $_form_resp==0 || $_form_resp==1)echo $noPermission;else{?>
<form id="sms_1583164170" autocomplete="off" action="" onsubmit="return submitter(this,'<?=isset($controllerURL)? $controllerURL:urlPanel?>');" method="post" enctype="multipart/form-data">
<input type="hidden" value="" name="e"/>
<input type="hidden" name="module" value="sms_1583164170"/> 
<input type="hidden" name="action" value="edit"/> 
<input type="hidden" name="id" value="<?=$id;?>"/> 

<!--inputs below -->
<!--
	

--><div class="form_field ontwo in  sms_1583164170_country_code" data-legion-field-type="number">
<label for="for_field_country_code"><?=l('Country Code<>المقدمة الدولية');?> <span class="required_star">*</span></label>
<div class="input_area">
<input id="for_field_country_code"  required type="number" name="country_code"   data-legion-module="sms_1583164170" placeholder="" value="<?=htmlspecialchars($_form_resp[0]['country_code']) ?>"/>
</div>
</div><!--


	

--><div class="form_field ontwo in  sms_1583164170_phone_number" data-legion-field-type="number">
<label for="for_field_phone_number"><?=l('Phone Number<>رقم الخلوي');?> <span class="required_star">*</span></label>
<div class="input_area">
<input id="for_field_phone_number"  required type="number" name="phone_number"   data-legion-module="sms_1583164170" placeholder="" value="<?=htmlspecialchars($_form_resp[0]['phone_number']) ?>"/>
</div>
</div><!--


	

--><div class="form_field  sms_1583164170_message" data-legion-field-type="textarea">
<label for="for_field_message"><?=l('Message<>الرسالة');?> <span class="required_star">*</span></label>
<div class="input_area">
<textarea  required  placeholder="" data-max_count="500"  class="mceNoEditor " name="message"><?=$_form_resp[0]['message'] ?></textarea>
</div>
</div><!--

-->
<!--inputs above -->
</form>
<?php } ?>