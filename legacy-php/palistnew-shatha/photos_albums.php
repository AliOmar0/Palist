    ...
    <div class="prx-hero">
        <div class="w1200 prx-hero-inner">
            ...
            <p class="prx-hero-sub">
                <?=l('Photo albums documenting the syndicate activities and events.<>ألبومات صور توثّق أنشطة النقابة وفعالياتها.')?>
            </p>
        </div>
    </div>
...
<<<<<<< HEAD
    ...
    if($resp==1) echo '<div class="no_data">'.l('No Data<>لا مُدخلات').'</div>';
            ...
            // echo $resp[$i]['id'];
            // echo "               ";
                ?>--><a class="photos_library_box in" title="<?=l($resp[$i]['title']) ?>"
                        href="<?=url($_m,'single',$resp[$i]['id'])?>">
                        <?php pic($resp[$i]['cover_photo'],400,100,l($resp[$i]['title']),true,NULL,'photos_library_cover_photo_picture');?>
                        <h2 class="photos_library_title"><?=l($resp[$i]['title']);?></h2>
                    </a>
                    <!--
        <?php 
            }//for
